...
/** Full visual editor for the Chronicle toast. */
public final class ToastCustomizerScreen extends Screen {
    ...
    /** Logical (unscrolled) Y positions for widgets in the scrolling editor. */
    ...
    protected void rebuildWidgets() {
        // clearWidgets() does not clear the focused-child reference in Screen.
        // A fullscreen/GUI-scale change can therefore leave the old EditBox
        // focused after the new layout is built. Reset the input state, let init
        // reconstruct the layout, then restore focus to the equivalent field.
    ...
    public void resize(int width, int height) {
        // init() already snapshots text from the old fields, so delegate through
        // the normal Screen resize path after explicitly ending transient drags.
        lastPressedAt = -1L;
        setDragging(false);
        draggingColorPicker = false;
        draggingHue = false;
    ...
    protected void init() {
        ...
        // 28px field + 8px gap before the next caption + 17px caption offset.
        ...
        // The complete palette card is the action. This removes the old visual
        // divider and makes the large, obvious target easier to use than a tiny USE button.
    ...
    /** A keyboard-skipped value display between the minus and plus actions. */
    ...
    private static int paletteBase(boolean compact, int controlWidth) {
        ...
        // Three two-column rows or six single-column rows, followed by enough
        // room for the last field and its PALETTE caption.
    ...
    private int reservedErrorStripHeight() {
        ...
        // Compact footers touch the viewport and need a dedicated strip. The
        // desktop layout already has a 12px non-content gutter above its footer.
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        ...
        int footerHeight = compactStyles
                ? (panelW < 260 ? 120 : ((panelW < 420 || this.height < 420) ? 48 : 56))
                : 66;
    ...
    /** Captures the layout positions before any scroll offset is applied. */
    ...
    /** Applies the same scroll offset to every editor control except the fixed footer. */
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        // Repaint scrolling buttons while the content scissor is still active.
        // Repainting them after disableScissor() allowed partially clipped cards
        // to leak over the header and footer.
        ...
        // The fixed footer is the only button layer painted above the content clip.
        for (var widget : this.children()) {
    ...
    private void drawFieldLabel(GuiGraphicsExtractor graphics, String text, EditBox box, int x, int color,
        ...
        // The label follows the actual, already-scrolled widget coordinate.
        // This prevents BACKGROUND / ACCENT / MESSAGE COLOR and the other field
        // captions from drifting away from their inputs during wheel scrolling.
    ...
    private void drawColorPicker(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
        ...
        // SV square: white -> pure hue horizontally, black overlay vertically.
        ...
        // Selection marker: classic compact cross, clamped inside the SV field.
        ...
        // Hue selection is a simple vertical slider marker.
        int hueX = width >= 5
                ? x + 2 + Math.round(pickerHue * (width - 5))
                : x + Math.max(0, width / 2);
        drawHueHandle(graphics, hueX, hueY + 5);

        // Current color swatch. Clamp it to the picker itself so a very narrow
        // GUI cannot paint the swatch into the neighbouring column.
    ...
    private int pickerHitMode(double mouseX, double mouseY) {
        ...
        boolean tinyCustomizer = panelW < 420 || this.height < 420;
        int clipTop = top + UiMetrics.HEADER_HEIGHT;
        int footerHeight = compact ? (panelW < 260 ? 120 : (tinyCustomizer ? 48 : 56)) : 66;
    ...
    private boolean updatePickerFromMouse(double mouseX, double mouseY, boolean allowOutside) {
        ...
        // While dragging, coordinates are intentionally clamped to the control
        // so the marker remains inside even when the cursor leaves its bounds.
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        // The fixed footer is visually above scrolling content, so dispatch it first.
