...
public final class NotificationSoundScreen extends Screen {
    ...
    /**
     * One geometry snapshot is shared by widget placement and rendering.  Keeping
     * captions and controls in separate calculations was the source of the
     * one/two-pixel overlaps visible at compact GUI scales.
     */
    ...
    protected void init() {
        ...
        // Keep the editable path useful before preserving a wide BROWSE label.
        // At normal sizes this resolves to the familiar 84/112px action, while
        // narrow GUI scales progressively shrink the action instead of crushing
        // the path field to one pixel.
    ...
    private SoundLayout soundLayout() {
        ...
        int titleY = top + (standard ? 16 : 9);
        int separatorY = top + (standard ? 55 : compact ? 31 : 25);
        int controlH;
        if (standard) {
            controlH = 30;
        } else if (compact) {
        ...
        if (standard) {
            ...
            // Compact screens keep the essential file caption, while MODE and
            // VOLUME remain self-describing inside their controls.
            modeLabelY = -1;
            ...
            // Emergency layout for very high GUI scales: all four control rows
            // remain reachable without letting captions collide with them.
            modeLabelY = -1;
            fileLabelY = -1;
    ...
    private int errorY(SoundLayout layout) {
        if (layout.formatsY() >= 0) {
            // On the desktop layout an error temporarily replaces the secondary
            // formats hint instead of adding a new line to an already full form.
        ...
        // At an extreme GUI scale there are no spare body lines; reuse the header
        // line and suppress its title for the duration of the actionable error.
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        if (layout.showSubtitle()) {
            graphics.text(this.font, Component.literal(UiFrame.trimToWidth(this.font,
                    ...
                    headerTextX, layout.top() + 35, MUTED, false);
        }
        graphics.fill(layout.x(), layout.separatorY(),
                layout.left() + layout.panelW() - layout.inset(),
                layout.separatorY() + 1, UiFrame.INNER_LINE);
    ...
    @Override
    public void onClose() {
        // A test may still be decoding or playing for up to 30 seconds. Leaving
        // the settings screen must cancel it instead of surprising the player.
        CustomSoundPlayer.stopCustom();
        transition.start(this.minecraft, parent);
    }
}
